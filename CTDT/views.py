            ...
            if tables: # Kiểm tra xem có bảng nào không
                ...
                for row in table.rows[1:]: # bắt đầu từ hàng thứ 2
                    ...
                    try:
                        ...
                        try:
                            if 'DC' in ghi_chu:
                                criterion1 = criterion.objects.get(title__iexact=tieu_chi)
                            else:
                        ...
                        if attest.objects.filter(attest_id=ma_minh_chung, body=so_ngay_ban_hanh, criterion=criterion1).exists():
                            ...
                            continue
                        if attest.objects.filter(attest_id=ma_minh_chung, criterion=criterion1).exists():
                            stt += 1
                        else:
                            stt = 1
                        # stt = (
                        ...
                        if 'DC' in ghi_chu:
                            ...
                            if not common_evidence:
                                messages.error(request, f"Minh chứng dùng chung '{ma_minh_chung}' chưa tồn tại. Vui lòng nhập minh chứng dùng chung trước khi tiếp tục.")
                                return render(request, 'admin/import_word.html')

                            # Kiểm tra nội dung dùng chung có khớp với nội dung từ Word không
                            fields_to_check = {
                                "title": title,
                                "body": so_ngay_ban_hanh,
                            ...
                            differences = {
                                field: (getattr(common_evidence, field), value)
                                for field, value in fields_to_check.items()
                                if getattr(common_evidence, field) != value
                            }

                            if differences:
                                messages.error(request, f"Có sự khác biệt giữa minh chứng dùng chung và dữ liệu Word:")
                                for field, (db_val, word_val) in differences.items():
                                    messages.error(request, f"{field}: Dùng chung = '{db_val}', Word = '{word_val}'")
                                return render(request, 'admin/import_word.html')

                            is_common1 = True
                            common_attest_instance = common_evidence
                        else:
                            is_common1 = False
                            common_attest_instance = None
                        ...
                        attest1 = attest(
                            ...
                            title=title,
                            box=box1,
                            criterion=criterion1,  # Liên kết với tiêu chí hiện tại
                            body=so_ngay_ban_hanh,
                            performer=noi_ban_hanh,
                            note = ghi_chu,
                            attest_stt = stt,
                            ...
                            common_attest = common_attest_instance
                        )
                        attest1.save()
                        print(attest1)
